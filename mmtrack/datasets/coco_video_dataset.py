...
class CocoVideoDataset(CocoDataset):
    ...
    def ref_img_sampling(self,
        ...
        if (not self.load_as_video) or img_info.get('frame_id', -1) < 0 \
            ...
            img_ids = self.coco.get_img_ids_from_vid(vid_id) # valid img ids in this video
            left = max(0, frame_id + frame_range[0]) # left bound
            right = min(frame_id + frame_range[1], len(img_ids) - 1) #right bound
            ...
            if method == 'uniform':
                ...
                if frame_id == 0:# new video
                    for i in range(frame_range[0], 1): # pad using img itself, and push self in
                        ref_img_ids.append(img_ids[0])
                    for i in range(1, frame_range[1] + 1):
                        ref_id = min(round(i * stride), len(img_ids) - 1)# using stride to sample ref img
                        ref_img_ids.append(img_ids[ref_id])
