...
class FlowNetSimpleOrigin(BaseModule):
    """Flow net copied from MEGA(https://github.com/Scalsol/mega.pytorch). 
    There seems something wrong with the mmtracking's default flownet on my custom datasets, all acc. are nearly 0.

    Args:
        BaseModule (_type_): _description_
    """

    def __init__(self, method='fgfa', init_cfg=None):
        super(FlowNetSimpleOrigin, self).__init__(init_cfg)
        ...
        self.Convolution2 = nn.Conv2d(
            1026, 2, kernel_size=3, stride=1, padding=1)
        self.Convolution3 = nn.Conv2d(
            770, 2, kernel_size=3, stride=1, padding=1)
        self.Convolution4 = nn.Conv2d(
            386, 2, kernel_size=3, stride=1, padding=1)
        self.Convolution5 = nn.Conv2d(
            194, 2, kernel_size=3, stride=1, padding=1)
        ...
        self.upsample_flow6to5 = nn.ConvTranspose2d(
            2, 2, kernel_size=4, stride=2)
        self.upsample_flow5to4 = nn.ConvTranspose2d(
            2, 2, kernel_size=4, stride=2)
        self.upsample_flow4to3 = nn.ConvTranspose2d(
            2, 2, kernel_size=4, stride=2)
        self.upsample_flow3to2 = nn.ConvTranspose2d(
            2, 2, kernel_size=4, stride=2)
